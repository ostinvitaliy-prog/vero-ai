...
async def analyze_and_style_news(title, description):
    prompt = (
        ...
        f"OUTPUT JSON ONLY:\n"
        f"{{\"score\": 1-10, \"ru\": \"text\", \"en\": \"text\", \"es\": \"text\", \"de\": \"text\", \"image_prompt\": \"futuristic crypto illustration\"}}"
    )
    ...
    payload = {
        "model": "gpt-4o-mini",
        "messages": [
            {"role": "system", "content": "You are a multilingual crypto journalist."},
...
async def image_generation(prompt: str):
    ...
    payload = {
        "model": "dall-e-3",
        "prompt": f"{prompt}, dark cyberpunk style, neon crypto, high quality, 16:9",
        "n": 1,
        "size": "1024x1024"
    }
