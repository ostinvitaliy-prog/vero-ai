...
async def analyze_news_ai(title, description, lang='ru'):
    prompt = f"Analyze crypto news in {lang}. Title: {title}. Description: {description}. Format: Title, Essence, Market Impact, VERO Verdict."
    
    headers = {"Authorization": f"Bearer {ROUTEL_API_KEY}", "Content-Type": "application/json"}
    payload = {
        "model": "gpt-4o-mini", # Используем экономную модель, чтобы точно хватило кредитов
